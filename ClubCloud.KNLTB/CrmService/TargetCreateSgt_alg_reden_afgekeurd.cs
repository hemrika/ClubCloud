using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Xml.Serialization;

namespace ClubCloud.KNLTB.ServIt.CrmService
{
	[DebuggerStepThrough]
	[DesignerCategory("code")]
	[GeneratedCode("System.Xml", "4.0.30319.33440")]
	[Serializable]
	[XmlType(Namespace="http://schemas.microsoft.com/crm/2007/WebServices")]
	public class TargetCreateSgt_alg_reden_afgekeurd : TargetCreate
	{
		private sgt_alg_reden_afgekeurd sgt_alg_reden_afgekeurdField;

		[XmlElement] //[XmlElement(Order=0)]
		public sgt_alg_reden_afgekeurd Sgt_alg_reden_afgekeurd
		{
			get
			{
				return this.sgt_alg_reden_afgekeurdField;
			}
			set
			{
				this.sgt_alg_reden_afgekeurdField = value;
				base.RaisePropertyChanged("Sgt_alg_reden_afgekeurd");
			}
		}

		public TargetCreateSgt_alg_reden_afgekeurd()
		{
		}
	}
}