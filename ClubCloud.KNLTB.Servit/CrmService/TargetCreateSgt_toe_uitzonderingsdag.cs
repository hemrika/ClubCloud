using System;
using System.CodeDom.Compiler;
using System.ComponentModel;
using System.Diagnostics;
using System.Xml.Serialization;

namespace ClubCloud.KNLTB.ServIt.CrmService
{
	[DebuggerStepThrough]
	[DesignerCategory("code")]
	[GeneratedCode("System.Xml", "4.0.30319.33440")]
	[Serializable]
	[XmlType(Namespace="http://schemas.microsoft.com/crm/2007/WebServices")]
	public class TargetCreateSgt_toe_uitzonderingsdag : TargetCreate
	{
		private sgt_toe_uitzonderingsdag sgt_toe_uitzonderingsdagField;

		[XmlElement] //[XmlElement(Order=0)]
		public sgt_toe_uitzonderingsdag Sgt_toe_uitzonderingsdag
		{
			get
			{
				return this.sgt_toe_uitzonderingsdagField;
			}
			set
			{
				this.sgt_toe_uitzonderingsdagField = value;
				base.RaisePropertyChanged("Sgt_toe_uitzonderingsdag");
			}
		}

		public TargetCreateSgt_toe_uitzonderingsdag()
		{
		}
	}
}