...
<asp:FormView runat="server" ID="EditGebruikerform" DefaultMode="Edit" RenderOuterTable="False" OnCallingDataMethods="EditGebruikerform_CallingDataMethods" SelectMethod="SelectGebruiker" UpdateMethod="UpdateGebruiker" DataKeyNames="Id">
    ...
    <EditItemTemplate>
        <fieldset>
            <h1 class="title-regular clearfix">
                <%# ((FormView)Container).CurrentMode == FormViewMode.Edit ? "Bewerken Gebruiker" : "" %>
            </h1>
            <asp:Button ID="Button1" runat="server" Text="Save" CommandName="Update" />
            <asp:Button ID="bn_goback" runat="server" Text="Terug naar Gebruikers" CausesValidation="false" OnClick="GoBack"/>
        </fieldset>
    </EditItemTemplate>
</asp:FormView>
