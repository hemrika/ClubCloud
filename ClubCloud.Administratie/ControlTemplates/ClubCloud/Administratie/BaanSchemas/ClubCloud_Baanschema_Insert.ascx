...
<asp:FormView runat="server" ID="InsertBaanschemaform" DefaultMode="Insert" RenderOuterTable="False" OnCallingDataMethods="InsertBaanschemaform_CallingDataMethods" SelectMethod="SelectBaanschema" InsertMethod="InsertBaanschema" DataKeyNames="Id">
    <EmptyDataTemplate>
    ...
    <InsertItemTemplate>
        <fieldset>
            <h1 class="title-regular clearfix">
                <%# ((FormView)Container).CurrentMode == FormViewMode.Insert ? "Nieuwe Baanschema" : "" %>
            </h1>
            <asp:Button ID="Insert" runat="server" Text="Save" CommandName="Insert" CausesValidation="true" />
            <asp:Button ID="Goback" runat="server" Text="Terug naar Baanschemas" CausesValidation="false" OnClick="GoBack"/>
        </fieldset>
    </InsertItemTemplate>
</asp:FormView>
    </contenttemplate>
