...
<asp:FormView runat="server" ID="EditAddressform" DefaultMode="Edit" RenderOuterTable="False" OnCallingDataMethods="EditAddressform_CallingDataMethods" SelectMethod="SelectAddress" UpdateMethod="UpdateAddress" DataKeyNames="Id" OnItemUpdated="UpdateAddressform_ItemUpdated">
    <EmptyDataTemplate>
        <h1 class="title-regular clearfix">
            Geen Address gevonden</h1>
        <div class="notice">
            Sorry, er is geen  Address beschikbaar voor dit Id.</div>
        <asp:LinkButton CssClass="button big" ID="btn_goback" CausesValidation="false" runat="server" Text="Terug naar Addressen"
            OnClick="GoBack" SkinID="Button" />
    </EmptyDataTemplate>
    <HeaderTemplate>
        <asp:LinkButton CssClass="button big" ID="btn_goback_top" runat="server" Text="Terug naar Address" CausesValidation="false" OnClick="GoBack" OnCommand="GoBack" CommandArgument='<%# Eval("Id") %>'/>
        <asp:LinkButton CssClass="button big" ID="btn_update_top" runat="server" Text="Save" CommandName="Update" CausesValidation="true" />
    </HeaderTemplate>
    <FooterTemplate>
        <asp:LinkButton CssClass="button big" ID="btn_goback_bottom" runat="server" Text="Terug naar Address" CausesValidation="false" OnClick="GoBack" OnCommand="GoBack" CommandArgument='<%# Eval("Id") %>'/>
    ...
    <EditItemTemplate>
        <fieldset>
			...
			Naam : <asp:DropDownList ID="Naam" runat="server" DataTextField="Value" DataValueField="Value" SelectedValue='<%# Bind("Naam") %>' SelectMethod="SelectNaam" OnCallingDataMethods="EditAddressform_CallingDataMethods" AppendDataBoundItems="true"><asp:ListItem  Text="Onbekend" Value="" Selected="True"></asp:ListItem> </asp:DropDownList></br>
            ...
            Land : <asp:DropDownList ID="LandId" runat="server" DataTextField="Naam" DataValueField="Id" SelectedValue='<%# Bind("LandId") %>' SelectMethod="SelectLand" OnCallingDataMethods="EditAddressform_CallingDataMethods" AppendDataBoundItems="true"><asp:ListItem  Text="Onbekend" Value="" Selected="True"></asp:ListItem> </asp:DropDownList></br>
			Actief : <asp:DropDownList ID="Actief" runat="server" DataTextField="Value" DataValueField="Value" SelectedValue='<%# Bind("Actief") %>' SelectMethod="SelectActief" OnCallingDataMethods="EditAddressform_CallingDataMethods" AppendDataBoundItems="true"><asp:ListItem  Text="Onbekend" Value="" Selected="True"></asp:ListItem> </asp:DropDownList></br>
